...
CREATE POLICY IF NOT EXISTS "opcionais_select_policy" ON opcionais
    FOR SELECT USING (true);

-- Criar política para permitir inserção/atualização para usuários autenticados
CREATE POLICY IF NOT EXISTS "opcionais_insert_policy" ON opcionais
    FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "opcionais_update_policy" ON opcionais
    FOR UPDATE USING (auth.role() = 'authenticated');

CREATE POLICY IF NOT EXISTS "opcionais_delete_policy" ON opcionais
