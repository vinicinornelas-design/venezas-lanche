-- Primeiro vamos limpar e reconfigurar tudo corretamente
...
-- Adicionar métodos de pagamento com fee_type correto (fixed ou percentage)
INSERT INTO payment_methods (nome, fee_type, fee_value) VALUES 
  ('Dinheiro', 'fixed', 0),
  ('PIX', 'fixed', 0),
  ('Cartão de Débito', 'fixed', 2.50),
  ('Cartão de Crédito', 'fixed', 3.50),
  ('Vale Refeição', 'fixed', 4.00)
ON CONFLICT DO NOTHING;
